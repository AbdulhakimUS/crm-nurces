...
function RiskBadge({ cat }) {
  ...
  return <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-xs font-semibold ${c.color}`}>{c.emoji} {c.label}</span>;
}
...
function StatusBadge({ status }) {
  ...
  return <span className={`px-2 py-0.5 rounded-full border text-xs font-semibold capitalize ${m[status]||'bg-gray-100 text-gray-600 border-gray-300'}`}>{status}</span>;
}

function Spinner() {
  return <div className="flex justify-center py-16"><div className="w-8 h-8 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"/></div>;
}

function Btn({ children, onClick, variant='primary', sm, type='button', disabled, full, className='' }) {
  const base = `inline-flex items-center justify-center gap-1.5 font-semibold rounded-xl transition-all disabled:opacity-50 ${sm?'px-3 py-1.5 text-xs':'px-4 py-2.5 text-sm'} ${full?'w-full':''} ${className}`;
  const v = {
...
function Inp({ label, ...p }) {
  return <Field label={label}><input className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent bg-white" {...p}/></Field>;
}

function Sel({ label, children, ...p }) {
  return <Field label={label}><select className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-white" {...p}>{children}</select></Field>;
}

function Tex({ label, ...p }) {
  return <Field label={label}><textarea className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 resize-none bg-white" rows={3} {...p}/></Field>;
}
...
function Modal({ open, onClose, title, children, wide }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className={`relative bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl w-full ${wide?'sm:max-w-2xl':'sm:max-w-md'} max-h-[92vh] flex flex-col`}>
...
function PatientForm({ initial, familyId, families, onSaved, onClose }) {
  ...
  const [form, setForm] = useState({
    ...
    risk_category: initial?.risk_category || 'normal',
  ...
  return (
    <div className="space-y-3">
      ...
      <Inp label="Full Name (F.I.SH)" value={form.full_name} onChange={set('full_name')} placeholder="Lastname Firstname Patronymic" required/>
      <Inp label="Passport ID" value={form.passport} onChange={set('passport')} placeholder="AA1234567" required/>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <Inp label="Blood Pressure" value={form.blood_pressure} onChange={set('blood_pressure')} placeholder="120/80"/>
      </div>
...
function PatientPage({ patientId, onBack, onDeleted }) {
  ...
  const del = async () => {
    if (!confirm('Delete this patient and all records?')) return;
  ...
  if (!data) return <div className="text-center py-16 text-gray-400">Patient not found</div>;

  const { patient, visits=[], history=[], vaccines=[], medications=[], images=[] } = data;
  const tabs = [
    { id:'info', label:'Info' }, { id:'history', label:'History' },
    { id:'visits', label:'Visits' }, { id:'vaccines', label:'Vaccines' },
    { id:'meds', label:'Meds' }, { id:'images', label:'Images' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gray-900 text-white px-4 pt-4 pb-5">
        <button onClick={onBack} className="flex items-center gap-1.5 text-indigo-300 hover:text-white text-sm font-medium mb-3 transition">
          ← Back
        </button>
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-0.5">
              ID: {patient.passport}{patient.birth_date && ` · DOB: ${fmt(patient.birth_date)}`}{patient.blood_group && ` · ${patient.blood_group}`}
            </p>
            <div className="flex flex-wrap gap-2 mt-2">
              ...
              {vaccines[0]?.next_date && isOverdue(vaccines[0].next_date) &&
                <span className="px-2 py-0.5 rounded-full bg-red-500/20 text-red-300 border border-red-500/30 text-xs font-semibold">Vaccine overdue</span>}
            </div>
          </div>
        </div>
        {/* Tabs */}
          ...
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition flex-shrink-0
                ${tab===t.id ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-white hover:bg-white/10'}`}>
              {t.label}
            </button>
          ))}
        </div>
      ...
      <div className="px-4 py-4 pb-24 space-y-3 max-w-2xl mx-auto">
          ...
          editMode ? (
            ...
            <>
              ...
              <button onClick={() => setEditMode(true)}
                className="w-full py-3.5 rounded-2xl bg-white border border-gray-200 text-gray-700 font-semibold text-sm hover:bg-gray-50 transition shadow-sm">
                ✏️ Edit Patient Info
              </button>
              <button onClick={del}
                className="flex items-center gap-2 text-red-500 font-semibold text-sm border border-red-200 rounded-2xl px-4 py-3 hover:bg-red-50 transition w-full justify-center">
                🗑️ Delete Patient
              </button>
            </>
          )
          ...
          <>
            {showHistory ? (
              <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-3">
                <h3 className="font-semibold text-gray-800">Add Visit Record</h3>
                ...
                <div className="flex gap-2">
                  ...
                  <Btn onClick={async()=>{await patientsApi.addHistory(patientId,histForm);toast.success('Added');setShowHistory(false);load();}} full>Save</Btn>
                </div>
              </div>
            ) : (
              <button onClick={()=>setShowHistory(true)} className="w-full py-3.5 rounded-2xl bg-white border border-dashed border-gray-300 text-gray-500 font-medium text-sm hover:bg-gray-50 transition">
                + Add Visit Record
              </button>
            )}
            {history.map(h => (
              <div key={h.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 relative">
                <button onClick={async()=>{await patientsApi.deleteHistory(patientId,h.id);load();}}
                  className="absolute top-3 right-3 text-gray-300 hover:text-red-500 text-lg transition">🗑</button>
        ...
        {tab==='visits' && (
          <>
            {showVisit ? (
              <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-3">
                <h3 className="font-semibold text-gray-800">Add Visit</h3>
                ...
                <div className="flex gap-2">
                  ...
                  <Btn onClick={async()=>{await patientsApi.addVisit(patientId,visitForm);toast.success('Added');setShowVisit(false);load();}} full>Save</Btn>
                </div>
              </div>
            ) : (
              <button onClick={()=>setShowVisit(true)} className="w-full py-3.5 rounded-2xl bg-white border border-dashed border-gray-300 text-gray-500 font-medium text-sm hover:bg-gray-50 transition">
                + Add Visit
              </button>
            )}
            {visits.map(v => (
              <div key={v.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-700 mt-0.5 flex items-center gap-2">
                    ...
                    {isOverdue(v.next_visit_date) && v.status !== 'completed' &&
                      <span className="px-2 py-0.5 rounded-full bg-red-50 text-red-500 border border-red-200 text-xs">overdue</span>}
                  </p>
                </div>
        ...
        {tab==='vaccines' && (
          <>
            {showVaccine ? (
              <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-3">
                <h3 className="font-semibold text-gray-800">Add Vaccination</h3>
                <Inp label="Vaccine Name" value={vacForm.vaccine_name} onChange={e=>setVacForm(p=>({...p,vaccine_name:e.target.value}))} placeholder="e.g. BCG, DTP, COVID-19"/>
                <div className="grid grid-cols-2 gap-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Btn onClick={async()=>{
                    if(!vacForm.vaccine_name){toast.error('Vaccine name required');return;}
                    await patientsApi.addVaccine(patientId,vacForm);toast.success('Added');setShowVaccine(false);load();
                  }} full>Save</Btn>
                </div>
              </div>
            ) : (
              <button onClick={()=>setShowVaccine(true)} className="w-full py-3.5 rounded-2xl bg-white border border-dashed border-gray-300 text-gray-500 font-medium text-sm hover:bg-gray-50 transition">
                + Add Vaccination
              </button>
            )}
            {vaccines.map(v => (
            ...
            {showMed ? (
              <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Btn onClick={async()=>{
                    if(!medForm.medicine_name){toast.error('Medicine name required');return;}
                    await patientsApi.addMedication(patientId,medForm);toast.success('Added');setShowMed(false);load();
                  }} full>Save</Btn>
                </div>
              </div>
            ) : (
              <button onClick={()=>setShowMed(true)} className="w-full py-3.5 rounded-2xl bg-white border border-dashed border-gray-300 text-gray-500 font-medium text-sm hover:bg-gray-50 transition">
                + Add Medication
              </button>
            )}
            {medications.map(m => (
              <div key={m.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    {m.quantity && `${m.quantity} · `}{m.times_per_day}x/day · {m.duration_days} days · {fmt(m.date_given)}
                  </p>
                </div>
          ...
          <>
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-3">
              ...
              <button onClick={()=>fileRef.current.click()}
                className="w-full py-3 rounded-xl bg-gray-50 hover:bg-gray-100 text-gray-600 font-medium text-sm transition border border-gray-200">
                📷 Upload Image
              </button>
            </div>
            <div className="grid grid-cols-2 gap-3">
            ...
            {!images.length && <div className="text-center py-12 text-gray-400 text-sm">No images uploaded</div>}
          </>
        )}
      </div>
    </div>
...
function FamilyPage({ familyId, onBack, onPatientSelect }) {
  ...
  const saveEdit = async () => {
    try { await familiesApi.update(familyId, editForm); toast.success('Updated'); setShowEdit(false); load(); }
    catch { toast.error('Failed'); }
  };

  const delFamily = async () => {
    if (!confirm('Delete this family and all members?')) return;
    try { await familiesApi.delete(familyId); toast.success('Deleted'); onBack(); }
    catch { toast.error('Failed'); }
  };

  if (loading) return <Spinner/>;
  if (!data) return <div className="text-center py-16 text-gray-400">Family not found</div>;
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="text-gray-400 text-sm mt-1 space-y-0.5">
              {family.address && <p>📍 {family.address}</p>}
              {family.phone && <p>📞 {family.phone}</p>}
            </div>
          </div>
          <div className="flex gap-2 mt-1">
            <button onClick={() => setShowEdit(true)} className="w-9 h-9 rounded-xl bg-white/10 hover:bg-white/20 flex items-center justify-center transition text-lg">✏️</button>
            <button onClick={delFamily} className="w-9 h-9 rounded-xl bg-red-500/20 hover:bg-red-500/30 flex items-center justify-center transition text-lg">🗑️</button>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-4 pb-24 max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-bold text-gray-900 text-lg">Members ({members.length})</h2>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    ...
                    {isOverdue(m.next_vaccine) && <span className="px-2 py-0.5 rounded-full bg-red-50 text-red-500 border border-red-200 text-xs font-semibold">Vaccine overdue</span>}
                  </div>
                </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {cards.map(c => (
          <button key={c.label} onClick={() => onNavigate(c.page, c.filter)}
            className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 text-left hover:border-indigo-200 hover:shadow-md transition">
            ...
            {stats.risk_groups.map(rg => {
              ...
              return (
                <button key={rg.risk_category} onClick={() => onNavigate('patients', rg.risk_category)}
                  className={`flex items-center justify-between w-full p-3 rounded-xl border ${cfg.color} hover:opacity-80 transition`}>
    ...
    <div className="space-y-4">
      ...
      <Modal open={showAdd} onClose={()=>setShowAdd(false)} title="Add Family">
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <Btn variant="outline" onClick={()=>setShowAdd(false)} full>Cancel</Btn>
            <Btn onClick={add} full>Save Family</Btn>
          </div>
        </div>
      </Modal>

      {families.length === 0 ? (
        <div className="text-center py-16 text-gray-400"><div className="text-5xl mb-3">🏠</div><p className="font-medium text-sm">No families yet</p></div>
      ) : (
        <div className="space-y-3">
          {families.map(f => (
...
function PatientsList({ initialRisk, initialVisit, onPatientSelect }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-black text-gray-900">Patients</h2>
      ...
      <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-2xl px-4 py-3 shadow-sm">
        ...
        <input className="flex-1 text-sm outline-none bg-transparent" placeholder="Search by name or passport..."
      ...
      {loading ? <Spinner/> : patients.length === 0 ? (
        <div className="text-center py-16 text-gray-400"><div className="text-5xl mb-3">👥</div><p className="font-medium text-sm">No patients found</p></div>
      ) : (
        <div className="space-y-2">
          {patients.map(p => (
...
function VaccinesPage({ onPatientSelect }) {
  ...
  useEffect(() => {
    // Get all patients then their vaccines
    patientsApi.list({}).then(async r => {
      const all = [];
      for (const p of r.data) {
        try {
          const vr = await patientsApi.getVaccines(p.id);
          vr.data.forEach(v => all.push({ ...v, patient_name: p.full_name, patient_id: p.id }));
        } catch {}
      }
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-2xl font-black text-gray-900">Vaccines</h2>
      {vaccines.length === 0 && <div className="text-center py-16 text-gray-400"><div className="text-5xl mb-3">💉</div><p className="text-sm font-medium">No vaccines recorded</p></div>}

      {overdue.length > 0 && (
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-red-100">
          <h3 className="font-bold text-red-600 mb-3 text-sm uppercase tracking-wide">⚠️ Overdue ({overdue.length})</h3>
          <div className="space-y-2">
            {overdue.map(v => (
              <div key={v.id} onClick={()=>onPatientSelect(v.patient_id)} className="flex items-center justify-between p-3 bg-red-50 rounded-xl border border-red-100 cursor-pointer hover:bg-red-100 transition">
                <div>
                  <p className="font-semibold text-gray-900 text-sm">💉 {v.vaccine_name}</p>
                  <p className="text-xs text-gray-500">{v.patient_name}</p>
                  <p className="text-xs text-red-500">Next: {fmt(v.next_date)}</p>
                </div>
                <span className="text-xs font-bold text-red-500 bg-red-100 px-2 py-0.5 rounded-full border border-red-200">Overdue</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {upcoming.length > 0 && (
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <h3 className="font-bold text-gray-700 mb-3 text-sm uppercase tracking-wide">📅 Upcoming ({upcoming.length})</h3>
          <div className="space-y-2">
            {upcoming.map(v => (
              <div key={v.id} onClick={()=>onPatientSelect(v.patient_id)} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100 cursor-pointer hover:bg-gray-100 transition">
                <div>
                  <p className="font-semibold text-gray-900 text-sm">💉 {v.vaccine_name}</p>
                  <p className="text-xs text-gray-500">{v.patient_name}</p>
                  <p className="text-xs text-gray-500">Next: {fmt(v.next_date)}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {noDate.length > 0 && (
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <h3 className="font-bold text-gray-700 mb-3 text-sm uppercase tracking-wide">✅ Given ({noDate.length})</h3>
          <div className="space-y-2">
            {noDate.map(v => (
              <div key={v.id} onClick={()=>onPatientSelect(v.patient_id)} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100 cursor-pointer hover:bg-gray-100 transition">
                <div>
                  <p className="font-semibold text-gray-900 text-sm">💉 {v.vaccine_name}</p>
                  <p className="text-xs text-gray-500">{v.patient_name} · Given: {fmt(v.date_given)}</p>
                </div>
              </div>
            ))}
          </div>
...
function ExportPage({ onBack }) {
  ...
  const doExport = () => {
    ...
    const rows = toExport.map(p => ({
      ...
      'Date of Birth': p.birth_date, 'Blood Group': p.blood_group,
      'Risk Category': p.risk_category, 'Family': p.family_name||'',
    ...
    if (format === 'csv') {
      ...
      a.download = `patients_${new Date().toISOString().split('T')[0]}.csv`; a.click();
    } else {
      const ws = XLSX.utils.json_to_sheet(rows);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Patients');
      const ext = format === 'xlsx' ? 'xlsx' : 'csv';
      XLSX.writeFile(wb, `patients_${new Date().toISOString().split('T')[0]}.${ext}`);
    }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="text-indigo-600 hover:text-indigo-800 font-semibold text-sm transition">← Back</button>
        <h2 className="text-2xl font-black text-gray-900">Export</h2>
      </div>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[['xlsx','📊 Excel'],['csv','📄 CSV'],['csv','📋 Numbers']].map(([val,label],i) => (
            <button key={i} onClick={()=>setFormat(val)}
              className={`p-3 rounded-xl border-2 text-sm font-semibold transition ${format===val&&i<2?'border-indigo-500 bg-indigo-50 text-indigo-700':'border-gray-200 text-gray-600 hover:border-gray-300'}`}>
              {label}
            </button>
          ))}
        </div>
      ...
      {/* Filters */}
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-3">
        <p className="text-sm font-semibold text-gray-700">Filter Patients</p>
        <div className="flex items-center gap-2 border border-gray-200 rounded-xl px-3 py-2.5">
          <span className="text-gray-400">🔍</span>
          <input className="flex-1 text-sm outline-none" placeholder="Search..." value={search} onChange={e=>setSearch(e.target.value)}/>
        </div>
        <select value={risk} onChange={e=>setRisk(e.target.value)}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm font-semibold text-gray-700">Select All ({patients.length})</span>
          </label>
            ...
            {patients.map(p => (
              <label key={p.id} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 cursor-pointer transition">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">{p.passport}{p.family_name&&` · ${p.family_name}`}</p>
                </div>
                {p.risk_category&&p.risk_category!=='normal'&&<RiskBadge cat={p.risk_category}/>}
              </label>
            ))}
      ...
      <Btn onClick={doExport} full className="py-4 text-base">
        📤 Export {selected.length>0?selected.length:patients.length} Patients
      </Btn>
    </div>
  );
}
...
function ImportPage({ onBack, onDone }) {
  ...
  const analyze = async (f) => {
    ...
    try {
      ...
      setAnalysis(r.data); setMapping(r.data.mapping || {});
      setStep('mapping');
  ...
  const process = async () => {
    ...
    try {
      ...
      setResults(r.data.results); setStep('results');
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="text-indigo-600 hover:text-indigo-800 font-semibold text-sm transition">← Back</button>
        <h2 className="text-2xl font-black text-gray-900">Import</h2>
      </div>

      {step==='upload' && (
        <>
          <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 text-sm text-blue-700">
            <p className="font-semibold mb-1">📋 How to prepare your Excel file:</p>
            <ul className="space-y-0.5 text-xs">
              <li>• Columns can be in any order — AI will detect them</li>
              <li>• Include: Full Name, Passport, Date of Birth, Blood Group</li>
              <li>• Optional: Family name, Address, Phone, Risk, Height, Weight</li>
          ...
          <div onClick={()=>fileRef.current.click()}
            ...
            <p className="font-bold text-gray-700">Tap to select Excel file</p>
          ...
          {loading && (
            <div className="text-center py-4">
              ...
              <p className="text-sm text-indigo-600 font-medium">Analyzing file...</p>
            </div>
          )}
        </>
      )}
      ...
      {step==='mapping' && analysis && (
        <>
          <div className="bg-green-50 border border-green-200 rounded-2xl p-3 text-sm text-green-700">
            ✓ Found {analysis.total_rows} rows with {analysis.headers?.length} columns. Review mapping below.
          </div>
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100">
              <p className="font-semibold text-gray-800 text-sm">Column Mapping</p>
              <p className="text-xs text-gray-400 mt-0.5">Match your columns to patient fields</p>
            </div>
            <div className="divide-y divide-gray-50 max-h-80 overflow-y-auto">
              {Object.entries(FIELD_LABELS).map(([field, label]) => (
                <div key={field} className="flex items-center gap-3 px-4 py-3">
                  <span className="text-xs font-bold text-indigo-600 w-32 flex-shrink-0">{label}</span>
      ...
      {step==='results' && results && (
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[['🏠 New Families',results.created_families,'text-indigo-600'],
              ['👥 New Patients',results.created_patients,'text-green-600'],
              ['✏️ Updated',results.updated_patients,'text-amber-600'],
              ['⚠️ Skipped',results.skipped,'text-red-500']].map(([l,v,c])=>(
              <div key={l} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          ...
          {results.errors?.length>0 && (
            <div className="bg-red-50 rounded-2xl p-3 border border-red-100 space-y-1">
              {results.errors.map((e,i)=><div key={i} className="text-xs text-red-600">Row {e.row}: {e.error}</div>)}
            </div>
          )}
    ...
    <div className="space-y-4 max-w-lg mx-auto">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            <button onClick={()=>fileRef.current.click()}
              className="absolute -bottom-1 -right-1 w-7 h-7 bg-indigo-600 text-white rounded-full text-xs flex items-center justify-center shadow hover:bg-indigo-700 transition">
              ✏️
            </button>
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Sel label="Theme" value={form.theme} onChange={e=>{setForm(p=>({...p,theme:e.target.value}));updateUser({theme:e.target.value});}}>
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={()=>setShowPw(p=>!p)} className="text-sm text-indigo-600 font-medium">{showPw?'Cancel':'Update'}</button>
        </div>
        {showPw && (
          <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <button onClick={onExport}
          className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 text-center hover:border-green-300 transition">
          <div className="text-2xl mb-1">📤</div>
          <p className="font-semibold text-sm text-gray-700">Export</p>
          <p className="text-xs text-gray-400">Excel/CSV</p>
        </button>
        <button onClick={onImport}
          className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 text-center hover:border-blue-300 transition">
          <div className="text-2xl mb-1">📥</div>
          <p className="font-semibold text-sm text-gray-700">Import</p>
      ...
      <button onClick={logout}
        className="w-full py-4 rounded-2xl border-2 border-red-200 text-red-500 font-bold hover:bg-red-50 transition text-sm">
        Sign Out
      </button>
    </div>
  );
...
export default function ClientDashboard() {
  ...
  const [patientsFilter, setPatientsFilter] = useState({ risk:'all', visit:'all' });
  ...
  const navigate = (pg, filter) => {
    if (pg === 'patients' && filter) {
      if (filter === 'missed') setPatientsFilter({ risk:'all', visit:'missed' });
      else setPatientsFilter({ risk:filter, visit:'all' });
    }
  ...
  const openPatient = (id) => { setSelectedPatientId(id); setPage('patient-detail'); };

  // Full-page routes (no nav)
  if (page === 'patient-detail' && selectedPatientId) {
    return <PatientPage patientId={selectedPatientId} onBack={()=>{ setPage(selectedFamilyId?'family-detail':'patients'); }} onDeleted={()=>setSelectedFamilyId(null)}/>;
  }
  if (page === 'family-detail' && selectedFamilyId) {
    return <FamilyPage familyId={selectedFamilyId} onBack={()=>setPage('families')} onPatientSelect={openPatient}/>;
  }
  if (page === 'export') {
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-2xl mx-auto px-4 py-6 pb-28">
          <ExportPage onBack={()=>setPage('profile')}/>
        </div>
      </div>
    );
  }
  if (page === 'import') {
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-2xl mx-auto px-4 py-6 pb-28">
          <ImportPage onBack={()=>setPage('profile')} onDone={()=>setPage('patients')}/>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-gray-900 text-white px-4 sm:px-6 py-3 flex items-center justify-between sticky top-0 z-40 shadow-xl">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-red-500 rounded-xl flex items-center justify-center text-lg shadow font-bold">🏥</div>
          <div>
            <p className="font-black text-base leading-tight">PatronageCare</p>
            <p className="text-xs text-gray-400">Home Visit System</p>
          </div>
        </div>
        <button onClick={()=>setPage('profile')} className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-gray-800 hover:bg-gray-700 transition text-sm font-medium">
          <span className="w-6 h-6 rounded-full bg-indigo-500 flex items-center justify-center text-xs font-bold overflow-hidden">
            {user?.photo_path ? <img src={user.photo_path} alt="" className="w-full h-full object-cover"/> : (user?.name||user?.login||'?')[0].toUpperCase()}
          </span>
          <span className="hidden sm:block max-w-24 truncate">{user?.name||user?.login}</span>
        </button>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-5 pb-28">
        ...
        {page==='patients' && <PatientsList initialRisk={patientsFilter.risk} initialVisit={patientsFilter.visit} onPatientSelect={openPatient}/>}
        {page==='vaccines' && <VaccinesPage onPatientSelect={openPatient}/>}
          ...
          {navItems.map(item => (
            <button key={item.id} onClick={()=>navigate(item.id)}
              className={`flex-1 flex flex-col items-center justify-center gap-0.5 py-3 transition-all ${
                page===item.id||( item.id==='families'&&page==='family-detail')||( item.id==='patients'&&(page==='patient-detail'||page==='vaccines'))
                  ? 'text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}>
              <div className={`p-1.5 rounded-xl transition-all ${page===item.id?'bg-indigo-50':''}`}>
