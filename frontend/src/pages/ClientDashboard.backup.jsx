...
function PatientPage() {
  ...
  const [progressForm, setProgressForm] = useState({ title: '', vaccine_type: '', description: '', record_date: new Date().toISOString().split('T')[0] });
  ...
  const fetchPatient = useCallback(async () => {
    try {
      ...
      setPatient(pRes.data); setForm(pRes.data); setProgress(prRes.data);
  ...
  const handleAddProgress = async () => {
    if (!progressForm.title || !progressForm.vaccine_type || !progressForm.description) { toast.error(t('patients.required')); return; }
    setSaving(true);
    try {
      ...
      setProgressForm({ title: '', vaccine_type: '', description: '', record_date: new Date().toISOString().split('T')[0] });
      setProgressPhoto(null); fetchPatient();
    } catch { toast.error(t('common.error')); } finally { setSaving(false); }
  };
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto">
      ...
      <div className="flex gap-2 mb-4">
        <button onClick={() => setTab('data')} className={"flex-1 py-2 rounded-lg text-sm font-medium transition-colors " + (tab === 'data' ? 'bg-primary-500 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300')}>{t('patients.data')}</button>
        <button onClick={() => setTab('progress')} className={"flex-1 py-2 rounded-lg text-sm font-medium transition-colors " + (tab === 'progress' ? 'bg-primary-500 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300')}>{t('patients.progress')}</button>
      </div>
      {tab === 'data' && (
      ...
      {tab === 'progress' && (
        <div className="flex flex-col gap-4">
          ...
          {progress.length === 0 ? <EmptyState message={t('progress.noRecords')} icon="💉" /> : (
            progress.map(r => (
              <Card key={r.id}>
                {r.photo_path && <img src={"/" + r.photo_path} alt={r.title} className="w-full h-48 object-cover rounded-lg mb-3" />}
                ...
                <span className="inline-block text-xs bg-blue-50 dark:bg-blue-900 text-blue-600 dark:text-blue-300 px-2 py-1 rounded mb-2">{r.vaccine_type}</span>
      ...
      <Modal open={addProgressModal} onClose={() => setAddProgressModal(false)} title={t('progress.addRecord')}>
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input type="file" accept="image/*" onChange={e => setProgressPhoto(e.target.files[0])} className="text-sm text-gray-500" />
            {progressPhoto && <p className="text-xs text-green-500">✓ {progressPhoto.name}</p>}
          </div>
          <div className="flex gap-3 justify-end">
            <Button variant="secondary" onClick={() => setAddProgressModal(false)}>{t('patients.cancel')}</Button>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-20">
      ...
      <main className="max-w-2xl mx-auto">
        <Routes>
          <Route index element={activeTab === 'patients' ? <PatientsList /> : <AddPatient />} />
